 ...
 * 
 * Test cases 1) Empty string 2) String where entire string is in dictionary 3)
 * String which cannot be split into words which are in dictionary 3) String
 * which can be split into words which are in dictionary
 *
 */
public class BreakMultipleWordsWithNoSpaceIntoSpace {
	...
	private List<String> wordBreakUtil(String s, Set<String> dict,
			Map<Integer, List<String>> dp, int start, int max) {
		if (start == s.length()) {
	...
	private boolean wordBreakTopDownOneSolutionUtil(String s, Set<String> dict,
			int start, int max, Map<Integer, Boolean> dp) {
		if (start == s.length()) {
			return true;
		}
